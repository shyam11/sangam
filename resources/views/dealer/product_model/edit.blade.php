        ...
        <form action="{{ route("admin.product-model.update") }}" method="POST" onSubmit="return confirm('Please verify your colors and addons before submit.')" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('category') ? 'has-error' : '' }}">
                ...
                <select name="category" id="category" class="form-control select2" required>
                    ...
                    <option value="almirah" @if($productModel->category == "almirah" ? 'selected' : '')@endif>Almirah</option>
                    <option value="office collection" @if($productModel->category == "office collection" ? 'selected' : '')@endif>Office Collection</option>
                </select>
            ...
            <div class="row">
                ...
                <div class="col-sm-4">
                    ...
                    @endforeach
                        <div class="form-check">
                            <input class="form-check-input" name="bodycolor[]" type="checkbox" value="{{$value->id}}" {{ $checked == 1 ? 'checked' : '' }}>
                            <label class="form-check-label" for="defaultCheck1">
                                {{$value->name}}
                            </label>
                        </div>
