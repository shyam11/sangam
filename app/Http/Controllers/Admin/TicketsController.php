...
class TicketsController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            // ->where('deleted_at',null)->where("tickets.status_id","!=",6)->get();
                ->where("tickets.status_id","!=",6)
                ->orderBy('tickets.created_at','DESC')
                ->filterTickets($request);
                $query->join('states','states.id','=','tickets.state_id')->join('cities','cities.id','=','tickets.city_id');
                 $id = Auth::user()->id;
                $user =User::where(["id"=>$id,"status"=>"active"])->first();
                if(!empty($user->city_id))
                {
                    $city = explode(',', $user->city_id);
                    $query = $query->whereIn("tickets.city_id",$city);
                }
                elseif(!empty($user->state_id))
                {
                    $state = explode(',', $user->state_id);
                    $query =  $query->whereIn("tickets.state_id",$state);
                }
                else{
                    $query = $query;
                }
                // return $queryObj;
                $query->select(sprintf('%s.*', (new Ticket)->table),'states.name as state_name','cities.city as city_name');
                // dd($query);
    ...
    public function create()
    {
        ...
        $states = getAllState();
        return view('admin.tickets.create', compact('statuses', 'priorities', 'categories', 'assigned_to_users','states'));
    }
    ...
    public function edit(Ticket $ticket)
    {
        ...
        $states = getAllState();
        $cities = getAllCities();
        return view('admin.tickets.edit', compact('statuses', 'priorities', 'categories', 'assigned_to_users', 'ticket','states','cities'));
    }
